...
import { CONTENT_CONFIG, INTERCEPT_CONFIG, BENIGN_ERROR_PATTERNS } from '../constants/config';
import '../styles/tailwind.css';

// Re-export for tests / CODE_DOCUMENTATION.md:15 reference
const ZERO_TARGET_MAX_ATTEMPTS = CONTENT_CONFIG.MAX_ZERO_TARGET_ATTEMPTS;
const ZERO_TARGET_WARN_AFTER_ATTEMPTS = CONTENT_CONFIG.WARN_AFTER_ZERO_TARGET_ATTEMPTS;
...
class ContentEngine {
  ...
  async init() {
    ...
    }, CONTENT_CONFIG.AUTO_ACTION_INTERVAL_MS);
  ...
  private async fetchLobbyData(matchId: string, forceRefresh = false) {
    ...
    this.loadTimer = setTimeout(() => {
      this.isLoading = false;
      this.renderAll();
    }, CONTENT_CONFIG.LOAD_TIMEOUT_MS);
  ...
  private handleFetchFailure(matchId: string) {
    ...
    if (this.retryCount < CONTENT_CONFIG.MAX_RETRIES) {
      this.retryCount++;
      this.retryTimer = setTimeout(() => {
        if (matchId === this.currentMatchId) {
          this.fetchLobbyData(matchId, true);
        }
      }, this.retryCount === 1 ? CONTENT_CONFIG.RETRY_DELAY_FIRST_MS : CONTENT_CONFIG.RETRY_DELAY_SECOND_MS);
    }
  }
  ...
  private handleInterceptedTraffic(p: InterceptedNetPayload) {
    ...
    if (!matchId) {
      ...
      const now = Date.now();
      if (now - this.lastProfileInterceptHandledAt < INTERCEPT_CONFIG.PROFILE_DEBOUNCE_MS) return;
    ...
    const now = Date.now();
    if (now - this.lastInterceptHandledAt < INTERCEPT_CONFIG.MATCH_DEBOUNCE_MS) return;
  ...
  private rememberObservedIdentity(guid: string, nickname?: string): void {
    ...
    while (this.observedIdentities.size > CONTENT_CONFIG.MAX_OBSERVED_IDENTITIES) {
  ...
  private renderPlayerBadges(allowTextFallback: boolean = true) {
    ...
    if (playerTargets.length === 0 && allRoster.length > 0) {
      ...
      if (
        ...
        const delayMs = attempt <= CONTENT_CONFIG.ZERO_TARGET_EARLY_RETRY_COUNT ? CONTENT_CONFIG.ZERO_TARGET_RETRY_EARLY_MS : CONTENT_CONFIG.ZERO_TARGET_RETRY_LATE_MS;
        this.zeroTargetRetryTimer = setTimeout(() => {
          ...
          if (
            ...
            this.renderPlayerBadges(attempt < CONTENT_CONFIG.TEXT_FALLBACK_DISABLE_AFTER);
          }
        }, delayMs);
      }
