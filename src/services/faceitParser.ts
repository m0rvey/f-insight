/**
 * FACEIT payload parsers — extracted from faceitApi.ts for modularity.
 * Pure functions, no network side-effects. Re-exported via faceitApi for backward compat.
 */
import {
  FaceitMatchDetails,
...
export function parsePlayerPayload(
  ...
  if (Array.isArray(history)) {
    for (let i = 0; i < history.length; i++) {
      ...
      if (i === 0) {
        ...
        if (res === currentStreakType) currentStreakCount++;
      ...
      const namedStats: Record<string, unknown> | null = item.stats && typeof item.stats === 'object' ? item.stats : null;
      const isPlausibleAdr = (v: number | undefined): v is number => v !== undefined && v >= 5 && v <= 200;
      ...
      if (mapName) {
        if (!historyMapStats[mapName]) historyMapStats[mapName] = { matches: 0, wins: 0, kills: 0, deaths: 0, adrSum: 0, adrCount: 0 };
      ...
      if (i < history.length - 1 && rawMatchElo) {
        ...
        if (typeof prevElo === 'number' && !isNaN(prevElo)) {
          ...
          if (Math.abs(diff) <= 60) eloDiff = diff;
        }
      }
      if (eloDiff === undefined) eloDiff = isWin ? 25 : -25;
  ...
  if (overallAdr === undefined) {
    ...
    if (adrMatches > 0) overallAdr = Math.round((adrWeightedSum / adrMatches) * 10) / 10;
  }
...
/**
 * Compose FaceitPlayerFullStats from payloads the page itself loaded (intercepted).
 */
