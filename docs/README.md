<!-- SYNC-NOTE: RU primary. Keep features/architecture/install in sync with docs/README_EN.md. See CONTRIBUTING.md: sync check `npm run check:docs`. -->
<div align="center">
...
### Быстрый способ — без Node.js

В репозитории уже лежит собранный бандл в папке [`dist/`](../dist/) — **intentionally tracked** (исключение из `GIT_AND_RELEASES.md:48`, см. `.gitignore:11`). Игрокам не нужен `npm`:

1. Скачайте ZIP актуальной ветки `main` (кнопка **Code → Download ZIP**) и распакуйте в простой путь без кириллицы и пробелов, например `C:\f-insight`.
2. Откройте `chrome://extensions/` → включите **Режим разработчика** → **Загрузить распакованное расширение** → выберите папку `dist`.
3. После обновления расширения достаточно нажать **Reload** на его карточке (если папка та же) или удалить и добавить её заново (если распаковали в новое место).

> `otherproject/` — локальные референсы чужих расширений для изучения оригинальных решений. В `.gitignore:5` и не коммитится, оставлен для удобства разработки.

### Сборка из исходного кода

Требуется **Node.js 18+** (рекомендуется актуальный LTS; проверьте `node -v`). Старые версии дадут непонятные ошибки esbuild.
